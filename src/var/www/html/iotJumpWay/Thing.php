...
$tid = filter_input(INPUT_GET, 'thing', FILTER_SANITIZE_STRING);
$thing = $iotJumpWay->get_thing($tid);
				...
				<div class="row">
					<div class="col-lg-8 col-md-12 col-sm-12 col-xs-12">
						<div class="panel panel-default card-view panel-refresh">
							<div class="panel-heading">
								<div class="pull-left">
									<h6 class="panel-title txt-dark">iotJumpWay Thing</h6>
								</div>
							...
							<div class="panel-wrapper collapse in">
								<div class="panel-body">
									<div class="form-wrap">
										<form data-toggle="validator" role="form" id="sensor_update_form" action="/iotJumpWay/Things/Update" method="POST" enctype="multipart/form-data" target="uploader">
											<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
												<div class="form-group">
													<label for="name" class="control-label mb-10">Name</label>
													<input type="text" class="form-control" id="name" name="name" placeholder="Things Name" required value="<?=$thing["name"]["value"]; ?>">
													<span class="help-block">Name of Thing</span>
												</div>
												<div class="form-group">
													<label for="name" class="control-label mb-10">Description</label>
													<input type="text" class="form-control" id="description" name="description" placeholder="Things Name" required value="<?=$thing["description"]["value"]; ?>">
													<span class="help-block">Description of Thing</span>
												</div>
												<div class="form-group">
													<label class="control-label mb-10">Category</label>
													<select class="form-control" id="category" name="category" required>
														<option value="sensor" <?=$thing["type"] == "sensor" ? " selected " : ""; ?>>sensor</option>
														<option value="actuator" <?=$thing["type"] == "actuator" ? " selected " : ""; ?>>actuator</option>
													</select>
													<span class="help-block">Sensor or Actuator</span>
												</div>
												<div class="form-group">
													<label for="name" class="control-label mb-10">Manufacturer</label>
													<input type="text" class="form-control" id="manufacturer" name="manufacturer" placeholder="Thing manufacturer" required value="<?=$thing["manufacturer"]["value"]; ?>">
													<span class="help-block">Name of thing manufacturer</span>
												</div>
												<div class="form-group">
													<label for="name" class="control-label mb-10">Model</label>
													<input type="text" class="form-control" id="model" name="model" placeholder="Thing model" required value="<?=$thing["model"]["value"]; ?>">
												...
												<div class="form-group">
													...
													<div id="propertyContent">
														<?php
															if(isSet($thing["properties"]["value"])):
																foreach($thing["properties"]["value"] AS $key => $value):
																	if($key != "image"):
														?>
												...
												<div class="form-group">
													...
													<div id="commandsContent">
														<?php
															if(isSet($thing["commands"]["value"])):
																foreach($thing["commands"]["value"] AS $key => $value):
																	if(is_array($value)):
																		$value = implode(',',$value);
																	endif;
														?>
												...
												<div class="form-group">
													...
													<?php
														if(isSet($thing["states"]["value"])):
															foreach($thing["states"]["value"] AS $key => $value):
													?>
												...
												<div class="form-group mb-0">
													<input type="hidden" name="thing" id="thing" value="<?=$thing["id"]; ?>" />
							...
							<div class="panel-wrapper collapse in">
								<div class="panel-body">
									<div style="height: 400px; overflow-y: scroll; overflow-x: hidden;">
										<?php echo "<pre id='schema'>"; ?> <?php print_r(json_encode($thing, JSON_PRETTY_PRINT)); ?> <?php echo "</pre>"; ?>
									</div>
								</div>
							</div>
						</div>
